...
const tokens = {
    ...
    ['+=']: (args, env) => {
        ...
        printErrors(`ReferenceError Tried incrementing an undefined variable: ${entityName}`, args);
        throw new ReferenceError(`Tried incrementing an undefined variable: ${entityName}`);
    },
    ['-=']: (args, env) => {
        if (args.length === 0 || args[0].type !== 'word') {
            printErrors('SyntaxError Invalid use of operation -=', args);
    ...
    ['->']: (args, env) => {
        ...
        const name = expr => {
            if (expr.type !== 'word') {
                printErrors('SyntaxError Arg names must be words', args.map(x => x.name));
        ...
        return (...args) => {
            if (args.length !== argNames.length) {
                printErrors('TypeError Invalid number of arguments near ("' +
                    argNames.join('; ') +
                    '")', args.map(x => x.name));
    ...
    ['=']: (args, env) => {
        ...
        printErrors(`ReferenceError Tried setting an undefined variable: ${entityName}`, args);
        throw new ReferenceError(`Tried setting an undefined variable: ${entityName}`);
    },
    ['.=']: (args, env) => {
        const main = args[0];
    ...
    ['::']: (args, env) => {
        try {
            let count = 0;
            return Object.fromEntries(args.reduce((acc, item, i) => {
                if (i % 2) {
                    acc[count].push(item.type === 'value' ? item.value : evaluate(item, env));
                    count++;
                }
                else {
                    if (typeof item.value !== 'string') {
                        printErrors('SyntaxError Invalid use of operation :: (Only strings can be used as keys)', args);
                        throw new SyntaxError('Invalid use of operation :: (Only strings can be used as keys)');
                    }
    ...
    ['.:']: (args, env) => {
        return args.map(item => item.type === 'value' ? item.value : evaluate(item, env));
    },
    // [':']: (args, env) => {
    ...
    // },
    ['<-']: (args, env) => exp => args.forEach(arg => {
        env[arg.name] = env[exp][arg.name];
    }) ?? VOID,
    ['|>']: (args, env) => {
        const [param, ...rest] = args;
        return pipe(...rest.map(arg => p => evaluate(arg, env)(p)))(param.type === 'apply' || param.type === 'word'
            ? evaluate(param, env)
            : param.value);
    }
    // ['|?|']: (args, env) =>
