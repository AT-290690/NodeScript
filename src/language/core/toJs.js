import { deps } from '../extentions/extentions.js';
...
export const toJavasScript = ({ source, env }) => {
  ...
  for (const moduleKey in env) {
    standartLibrary += `"${moduleKey}":{`;
    for (const method of env[moduleKey]) {
      standartLibrary += `"${method}":`;
      const currentDep = deps[moduleKey][method];
      if (typeof currentDep === 'function') {
        standartLibrary += currentDep.toString().replace('VOID', 'null');
      } else {
        standartLibrary += JSON.stringify(currentDep);
      }
      standartLibrary += ',';
    }
    standartLibrary += '},';
